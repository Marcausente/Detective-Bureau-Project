    ...
    const handleSendTaskToBoard = async (task, catName) => {
        try {
            ...
            if (existing && existing.length > 0) {
                alert(language === 'es' ? 'Esta tarea ya está en la pizarra del caso.' : 'This task is already on the case whiteboard.');
                return;
            }
                            ...
                            {editingCatId === cat.id ? (
                                ...
                                <>
                                    ...
                                    <div className="column-actions" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                        ...
                                        >
                                            📌 {t('addToBoardBtn') || 'A Pizarra'}
                                        </button>
                                        <button onClick={() => startEditCategory(cat)} title={t('renameTooltip')} style={{ background: 'none', border: 'none', cursor: 'pointer', opacity: 0.5 }}>✎</button>
