...
function Dashboard() {
    ...
    const [newPost, setNewPost] = useState({ title: '', content: '', pinned: false, images: [], sendToDiscord: true });
    ...
    const handleSaveAnnouncement = async (e) => {
        ...
        try {
            ...
            if (editingId) {
                ...
                // Send notification via Discord Webhook if active
                if (newPost.sendToDiscord !== false) {
                    sendAnnouncementToDiscord({
                        ...
                        console.warn('Discord Webhook notification skipped or failed:', discordErr);
                    });
                }
            }
    ...
    const closeModal = () => {
        setShowModal(false);
        setNewPost({ title: '', content: '', pinned: false, images: [], sendToDiscord: true });
        setEditingId(null);
    };
                        ...
                        <form onSubmit={handleSaveAnnouncement} style={{ padding: '1.75rem' }}>
                            ...
                            {!editingId && (
                                <div className="form-group" style={{
                                    ...
                                    <input
                                        type="checkbox"
                                        id="discordNotifyCheck"
                                        checked={newPost.sendToDiscord !== false}
                                        onChange={e => setNewPost({ ...newPost, sendToDiscord: e.target.checked })}
                                        style={{ cursor: 'pointer', width: '16px', height: '16px' }}
                                    />
                                    <label htmlFor="discordNotifyCheck" className="form-label" style={{ margin: 0, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.86rem', color: '#cbd5e1' }}>
                                        ...
                                        <span>Notificar y publicar automáticamente en <strong>Discord</strong></span>
                                    </label>
                                </div>
                            )}
