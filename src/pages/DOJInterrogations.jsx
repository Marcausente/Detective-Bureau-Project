...
function DOJInterrogations() {
    ...
    const loadData = async () => {
        try {
            setLoading(true);
            const { data, error } = await supabase.rpc('get_doj_interrogations', { p_case_id: null }); // Get all
            if (error) throw error;
            setInterrogations(data || []);
        } catch (err) {
            console.error('Error loading DOJ interrogations:', err);
        } finally {
            setLoading(false);
        }
    };
                    ...
                    {filteredItems.length === 0 ? (
                        ...
                        filteredItems.map(item => (
                            <div key={item.id} className="interrogation-card" style={{ borderLeft: '4px solid #3b82f6' }}>
                                ...
                                {item.meddoj_url && (
                                    <a href={item.meddoj_url} target="_blank" rel="noopener noreferrer" className="int-link-btn">
...
export default DOJInterrogations;
