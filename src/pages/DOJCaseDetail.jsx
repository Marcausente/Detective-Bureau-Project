...
import '../index.css';
import DOJCaseTodoList from '../components/DOJCaseTodoList'; // Import component
import ReactQuill from 'react-quill';
import 'react-quill/dist/quill.snow.css';

function DOJCaseDetail() {
    const { id } = useParams();
    ...
    const loadAvailableInterrogations = async () => {
        try {
            const { data, error } = await supabase.rpc('get_available_doj_interrogations_to_link');
    ...
    const handleLinkInterrogation = async (interrogationId) => {
        try {
            const { error } = await supabase.rpc('manage_doj_interrogation', {
                p_action: 'link',
    ...
    const loadCaseDetails = async () => {
        setLoading(true);
        const { data, error } = await supabase.rpc('get_doj_case_details', { p_case_id: id });
        if (error) {
            console.error('Error loading DOJ case:', error);
    ...
    const handleUpdateAssignments = async () => {
        try {
            const { error } = await supabase.rpc('update_doj_case_assignments', {
                p_case_id: id,
    ...
    const handlePostUpdate = async (e) => {
        ...
        try {
            const { error } = await supabase.rpc('add_doj_case_update', {
                p_case_id: id,
    ...
    const handleDeleteUpdate = async (updateId) => {
        ...
        try {
            const { error } = await supabase.rpc('delete_doj_case_update', { p_update_id: updateId });
            if (error) throw error;
            loadCaseDetails();
        } catch (err) {
        ...
        <div className="documentation-container" style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem' }}>
            <div className="case-detail-header" style={{ borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '1rem', marginBottom: '2rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <button onClick={() => navigate('/doj/cases')} style={{ background: 'none', border: 'none', color: 'var(--accent-gold)', cursor: 'pointer', marginBottom: '1rem' }}>
                        ← Back to DOJ Cases
                    </button>
                </div>
                ...
                <div style={{ marginTop: '1.5rem', padding: '1rem', background: 'rgba(0,0,0,0.3)', borderRadius: '8px', borderLeft: '4px solid var(--accent-gold)' }}>
                    <h4 style={{ margin: '0 0 0.5rem 0', color: 'var(--accent-gold)' }}>INITDOJL REPORT</h4>
