...
function DOJCases() {
    ...
    const [users, setUsers] = useState([]); // DOJ Users for assignment
    ...
    const fetchCases = async () => {
        setLoading(true);
        const { data, error } = await supabase.rpc('get_doj_cases', { p_status_filter: filter });
        if (error) console.error('Error fetching DOJ cases:', error);
        else setCases(data || []);
        setLoading(false);
    };
    ...
    const handleCreateCase = async (e) => {
        ...
        try {
            ...
            navigate(`/doj/cases/${newId}`);

        } catch (err) {
            alert('Error creating DOJ case: ' + err.message);
        } finally {
            setSubmitting(false);
        }
    };
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', marginTop: '2rem' }}>
                    {cases.map(c => (
                        <div
                            ...
                            onClick={() => navigate(`/doj/cases/${c.id}`)}
                        >
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                                <span style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>DOJ-CASE #{String(c.case_number).padStart(3, '0')}</span>
            ...
            {showCreateModal && (
                <div className="cropper-modal-overlay">
                    <div className="cropper-modal-content" style={{ maxWidth: '700px', textAlign: 'left', maxHeight: '90vh', overflowY: 'auto' }}>
                        <h3 className="section-title">Open New DOJ Investigation</h3>
                        <form onSubmit={handleCreateCase}>
                            <div className="form-group">
                                <label className="form-label">Case Title</label>
                                <input type="text" className="form-input" required
                                    value={newCase.title} onChange={e => setNewCase({ ...newCase, title: e.target.value })} placeholder="e.g. Federal Investigation - 418" />
                            </div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                <div className="form-group">
                                    ...
                                    <input type="text" className="form-input" required
                                        value={newCase.location} onChange={e => setNewCase({ ...newCase, location: e.target.value })} placeholder="e.g. Federal Building" />
                                </div>
                                <div className="form-group">
                            ...
                            <div className="form-group">
                                <label className="form-label">Assign DOJ Agents</label>
                                <div style={{ maxHeight: '150px', overflowY: 'auto', background: 'rgba(0,0,0,0.3)', padding: '0.5rem', borderRadius: '4px', border: '1px solid var(--glass-border)' }}>
                                    {users.length === 0 ? <div style={{ color: '#aaa', padding: '1rem' }}>No DOJ agents found.</div> :
                                        users.map(u => (
                                            <div key={u.id}
