...
CREATE OR REPLACE FUNCTION create_doj_case(
  p_title TEXT,
  p_location TEXT,
...
DECLARE
  ...
  v_uid UUID;
BEGIN
  INSERT INTO public.doj_cases (title, location, occurred_at, description, initial_image_url, created_by)
  ...
  IF p_assigned_ids IS NOT NULL THEN
    ...
    LOOP
      INSERT INTO public.doj_case_assignments (case_id, user_id) VALUES (v_new_case_id, v_uid);
    END LOOP;
  END IF;
...
CREATE OR REPLACE FUNCTION get_doj_cases(p_status_filter TEXT DEFAULT NULL)
RETURNS TABLE (
  id UUID,
  case_number INT,
  title TEXT,
...
CREATE OR REPLACE FUNCTION update_doj_case_assignments(p_case_id UUID, p_assigned_ids UUID[])
RETURNS VOID AS $$
DECLARE
  v_uid UUID;
BEGIN
  ...
  IF p_assigned_ids IS NOT NULL THEN
    FOREACH v_uid IN ARRAY p_assigned_ids
    LOOP
       INSERT INTO public.doj_case_assignments (case_id, user_id) VALUES (p_case_id, v_uid);
    END LOOP;
  END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
RETURNS VOID AS $$
BEGIN
  INSERT INTO public.doj_sanctions (target_user_id, author_user_id, sanction_type, amount, reason)
  VALUES (p_target_id, auth.uid(), p_type, p_amount, p_reason);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

CREATE OR REPLACE FUNCTION get_doj_sanctions()
RETURNS TABLE (
  id UUID,
  sanction_type TEXT,
  amount INT,
...
-- =============================================
-- DOJ INTERROGATIONS RPCS
-- =============================================

CREATE OR REPLACE FUNCTION create_doj_interrogation(
  p_title TEXT,
  p_summary TEXT,
  p_subjects TEXT,
...
DECLARE
  v_id UUID;
BEGIN
  INSERT INTO public.doj_interrogations (title, summary, subjects, case_id, created_by)
...
CREATE OR REPLACE FUNCTION get_doj_interrogations()
RETURNS TABLE (
  id UUID,
  title TEXT,
  subjects TEXT,
...
BEGIN
  ...
  FROM public.doj_interrogations i
  LEFT JOIN public.doj_cases c ON i.case_id = c.id
